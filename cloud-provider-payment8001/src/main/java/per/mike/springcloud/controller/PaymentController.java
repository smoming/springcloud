...
public class PaymentController {
	...
	public CommonResult<Object> getDiscovery() {
		...
		for (String svc : discoveryClient.getServices()) {
			...
			for (ServiceInstance ins : discoveryClient.getInstances(svc)) {
				log.info(String.join("\t", ins.getServiceId(), ins.getHost(), String.valueOf(ins.getPort()),
						ins.getUri().toString()));
			}
		}
	...
	public CommonResult<String> zipkin() {
		return new CommonResult<String>(200, "調用成功 for Zipkin", serverPort);
	}
}
